...
namespace AzureBusDepot.UnitTests
{
    public class SingleMessageTypeListenerShould
    {
        ...
        private readonly SpyLogger<SingleMessageTypeListener<MyEndpointHandlingConfig, MyEvent>> _spyLogger;
        ...
        public async Task Track_Request_Telemetry_When_Handling_Messages(int count, bool isSuccessful, string customProperty)
        {
            ...
            _mockInstrumentor.Verify(
                i => i.TrackRequest(
                    LogEventIds.ListenerHandlerFinished, It.IsAny<long>(), It.IsAny<DateTimeOffset>(), "SingleMessageTypeListener<MyEndpointHandlingConfig,MyEvent>", null, null, isSuccessful, It.Is<IDictionary<string, object>>(d => (string)d["customProperty"] == customProperty)),
                Times.Exactly(count));
        }
